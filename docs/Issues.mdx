...
### 当前状态
现在是既没有喜悦也没有懊恼，只是无感，人麻了。关于此问题的进一步记录明天再来弄吧，睡觉先。
...
## Docusaurus 多语言网站文档页面评论共享问题

### 问题描述
Docusaurus 多语言网站中，文档页面的中英文评论无法共享，而博客页面可以正常共享。

### 核心问题
路径处理逻辑未能正确移除文档页面的语言前缀，导致中英文页面使用不同的 `term` 参数：
- 中文页面：`docs/mydoc/testpage2`
- 英文页面：`en/docs/mydoc/testpage2`（应为 `docs/mydoc/testpage2`）

### 调试发现
1. 已实现博客页面评论共享，但原因不明确（`term` 不同但评论共享）
2. 文档页面因 `term` 参数不一致导致评论无法共享
3. 多种路径处理方法均未能解决文档页面问题

当前功能已满足基本需求，后续可深入研究路径处理问题。

## Docusaurus 站点标题和标语国际化问题总结

### 问题描述
无法成功翻译站点的 title 和 tagline，中英文页面显示相同内容。

### 尝试过的解决方案
1. 创建 `i18n/zh-Hans/site.json` 和 `i18n/en/site.json` 翻译文件
2. 使用函数形式配置 title 和 tagline（导致验证错误）
3. 完整重启开发环境和清除缓存
4. 使用 `yarn write-translations` 命令
