# 💡 嵌入式技术博客

> 📚 基于 Docusaurus 构建的嵌入式开发学习笔记与技术分享平台

![Docusaurus](https://img.shields.io/badge/Docusaurus-3.0+-blue?style=for-the-badge&logo=docusaurus)
![React](https://img.shields.io/badge/React-18+-61dafb?style=for-the-badge&logo=react)
![MDX](https://img.shields.io/badge/MDX-v3-yellow?style=for-the-badge&logo=mdx)
![License](https://img.shields.io/badge/License-CC%20BY--NC--SA%204.0-orange?style=for-the-badge&logo=creative-commons)

---

## 🎯 关于本站

这是一个专注于**嵌入式系统开发**的技术博客，记录我在嵌入式领域的学习心得与实战经验。

**主要内容方向：**

- 🔌 **微控制器开发**：STM32、ESP8266/ESP32 等项目实战
- 📡 **物联网技术**：传感器应用、无线通信、IoT 方案设计
- 💻 **编程能力**：数据结构与算法题解、编程技巧
- 🛠️ **工程实践**：开发环境搭建、调试技巧、项目踩坑记录

---

## 👨‍💻 关于作者

- 🔭 目前专注于：嵌入式系统开发与物联网技术应用
- 🌱 正在学习：RTOS
- 📫 访问我的博客：[https://eurekax.space](https://eurekax.space)

---

## 📄 开源协议

本项目采用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/) 协议授权。

内容多为学习笔记和个人理解，欢迎交流分享。如需转载或引用，请署名并保持相同方式共享。详细条款请查看 [LICENSE](./LICENSE)
文件。
